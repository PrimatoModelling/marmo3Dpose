...
# tkaneko -------------------------------
class OmnidirCamera(Camera):
    def set_xi(self, xi):
        self.xi = np.array(xi, dtype='float64').ravel()
    def get_xi(self):
        return self.xi
    def set_K(self, K):
    ...
    def get_K(self):
        return self.K
    def set_D(self, D):
        self.D = np.array(D, dtype='float64').ravel()
    def get_D(self):
        return self.D
    def load_dict(self, d):
        self.set_camera_matrix(d['matrix'])
    ...
    def distort_points(self, points):
        ...
        points = points.reshape(-1, 1, 2)
        new_points = np.dstack([points, np.ones((points.shape[0], 1, 1))])
        out, _ = cv2.omnidir.projectPoints(new_points,
                                           np.zeros(3), np.zeros(3),
                                           self.K.astype('float64'),
                                           self.xi.astype('float64')[0],  
    ...
    def undistort_points(self, points):
        shape = points.shape
        points = points.reshape(-1, 1, 2)
        out = cv2.omnidir.undistortPoints(points.astype('float64'),
                                          self.K.astype('float64'),
                                          self.D.astype('float64'),
                                          self.xi.astype('float64'),                                          
                                          np.eye(3)) 
    ...
    def project(self, points):
        points = points.reshape(-1, 1, 3)  
        out, _ = cv2.omnidir.projectPoints(points,
                                           self.rvec, self.tvec,
                                           self.K.astype('float64'),
                                           self.xi.astype('float64')[0],  
                                           self.D.astype('float64'))            
...
class CameraGroup:
    ...
    def triangulate(self, points, undistort=True, progress=False):
        ...
        if undistort:# tkaneko test purpose         
    ...
    def optim_points(self, points, p3ds,
        ...
        return p3ds_new2, joint_len

    def optim_points_jointlenfix(self, points, p3ds, joint_len,
                     constraints=[],
                     constraints_weak=[],
                     scale_smooth=4,
                     scale_length=2, scale_length_weak=0.5,
        ...
        jac = self._jac_sparsity_triangulation_jointlenfix(
            points, constraints, constraints_weak, n_deriv_smooth)

        opt2 = optimize.least_squares(self._error_fun_triangulation_jointlenfix,
                                      x0=x0, jac_sparsity=jac,
                                      loss='linear',
                                      ftol=1e-3,
                                      max_nfev=15,
                                      verbose=2*verbose,
                                      args=(points, joint_len,
                                            constraints,
                                            constraints_weak,
                                            scores,
                                            scale_smooth_full,
    ...
    def _jac_sparsity_triangulation_jointlenfix(self, p2ds,
                                    constraints=[],
                                    constraints_weak=[],
                                    n_deriv_smooth=1):
    ...
    def _error_fun_triangulation_jointlenfix(self, params, p2ds, joint_len,
                                 constraints=[],
                                 constraints_weak=[],
                                 scores=None,
                                 scale_smooth=10000,
        ...
        joint_lengths = joint_len[:n_constraints]
        joint_lengths_weak = joint_len[n_constraints:]
    ...
    def from_dicts(arr):
        ...
        for d in arr:
            ...
            elif 'omnidir' in d and d['omnidir']:
                cam = OmnidirCamera.from_dict(d)
            else:
